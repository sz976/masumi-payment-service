...
import { HotWalletType, RPCProvider, PaymentType, Network } from '@prisma/client';
...
export const paymentSourceExtendedCreateSchemaInput = z.object({
    ...
    paymentType: z.nativeEnum(PaymentType).describe("The type of payment source used"),
    PaymentSourceConfig: z.object({
    ...
    feeRatePermille: z.number({ coerce: true }).min(0).max(1000).describe("The fee in permille to be used for the payment source. The default contract uses 50 (5%)"),
    AdminWallets: z.array(z.object({
        ...
        return await prisma.$transaction(async (prisma) => {
            ...
            const sellingWallets = await Promise.all(sellingWalletsMesh.map(async (sw) => {
                return {
                    ...
                    secretId: (await prisma.walletSecret.create({ data: { encryptedMnemonic: sw.mnemonicEncrypted } })).id,
                    note: sw.note,
            ...
            const purchasingWallets = await Promise.all(purchasingWalletsMesh.map(async (pw) => {
                return {
                    ...
                    secretId: (await prisma.walletSecret.create({ data: { encryptedMnemonic: pw.mnemonicEncrypted } })).id,
                    note: pw.note,
        ...
        const result = await prisma.$transaction(async (prisma) => {
            ...
            const paymentSource = await prisma.paymentSource.update({
                ...
                data: {
                    ...
                    PaymentSourceConfig: input.PaymentSourceConfig != null ? {
                        update: { rpcProviderApiKey: input.PaymentSourceConfig.rpcProviderApiKey }
                    } : undefined,
                    HotWallets: {
                        createMany: {
                            data: [...purchasingWallets, ...sellingWallets]
