...
import { z } from 'zod';
import { HotWalletType, Network, OnChainState, PaymentAction, PaymentErrorType, PaymentType, } from '@prisma/client';
...
export const queryPaymentsSchemaInput = z.object({
    ...
    smartContractAddress: z.string().max(250).optional().describe("The address of the smart contract where the payments were made to"),
    includeHistory: z.string().optional().transform(val => val?.toLowerCase() == "true").default("false").describe("Whether to include the full transaction and status history of the payments")
})
...
export const queryPaymentEntryGet = readAuthenticatedEndpointFactory.build({
    ...
    handler: async ({ input, options }) => {
        const paymentSourceAddress = input.smartContractAddress ?? (input.network == Network.Mainnet ? DEFAULTS.PAYMENT_SMART_CONTRACT_ADDRESS_MAINNET : DEFAULTS.PAYMENT_SMART_CONTRACT_ADDRESS_PREPROD)
...
export const paymentInitPost = readAuthenticatedEndpointFactory.build({
    ...
    handler: async ({ input, options }) => {
        ...
        const sellingWallet = specifiedPaymentContract.HotWallets.find(wallet => wallet.walletVkey == vKey && wallet.type == HotWalletType.Selling)
