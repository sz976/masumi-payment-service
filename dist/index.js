...
import {
  resolvePaymentKeyHash as resolvePaymentKeyHash$1,
  resolveStakeKeyHash,
...
const CONFIG = {
  ...
  CHECK_COLLECTION_INTERVAL:
    process.env.CHECK_COLLECTION_INTERVAL ?? '*/5 * * * *',
  CHECK_COLLECT_REFUND_INTERVAL:
    process.env.CHECK_COLLECT_REFUND_INTERVAL ?? '*/5 * * * *',
  ...
  CHECK_WALLET_TRANSACTION_HASH_INTERVAL:
    process.env.CHECK_WALLET_TRANSACTION_HASH_INTERVAL ?? '*/1 * * * *',
...
function buildDevLogger() {
  const logFormat = printf(
    ({ level, message, timestamp: timestamp2, stack }) => {
...
async function checkLatestTransactions() {
  ...
  if (!acquiredMutex) return await updateMutex$6.acquire();
  try {
    const networkChecks = await prisma.$transaction(
      async (prisma2) => {
        ...
        if (networkChecks2.length == 0) {
          logger.warn(
            'No available network handlers found, skipping update. It could be that an other instance is already updating',
          );
          return null;
        }
    ...
    if (networkChecks == null) return;
    try {
      const results = await Promise.allSettled(
        networkChecks.map(async (networkCheck) => {
          ...
          const blockfrost = new BlockFrostAPI({
            ...
            network:
              networkCheck.network == $Enums.Network.MAINNET
                ? 'mainnet'
                : 'preprod',
          });
          let latestTx = await blockfrost.addressesTransactions(
            networkCheck.paymentContractAddress,
            { count: 25, page: networkCheck.lastPageChecked },
          );
          while (latestTx.length > 0) {
            const foundTxIndex = latestTx.findIndex(
              (tx) => tx.tx_hash == latestIdentifier,
            );
            if (foundTxIndex == latestTx.length - 1) break;
            if (foundTxIndex != -1) latestTx = latestTx.slice(foundTxIndex);
            const txData = await Promise.all(
              latestTx.map(async (tx) => {
                try {
                  ...
                  logger.warn('Error getting tx metadata, ignoring tx', {
                    error,
                    tx: tx.tx_hash,
                  });
            ...
            const filteredTxData = txData.filter(
              (x) => x.utxos != null && x.transaction != null,
            );
              ...
              if (redeemers == null) {
                ...
                for (const output of valueOutputs) {
                  ...
                  await prisma.$transaction(
                    async (prisma2) => {
                      const databaseEntry =
                        await prisma2.purchaseRequest.findMany({
                          where: {
                            identifier: decodedNewContract.referenceId,
                            networkHandlerId: networkCheck.id,
                            status:
                              $Enums.PurchasingRequestStatus.PurchaseInitiated,
                          },
                        });
                        ...
                        for (const entry of databaseEntry) {
                          await prisma2.purchaseRequest.update({
                            ...
                            data: {
                              ...
                              errorNote: 'Duplicate purchase transaction',
                      ...
                      await prisma2.purchaseRequest.update({
                        ...
                        data: {
                          status:
                            $Enums.PurchasingRequestStatus.PurchaseConfirmed,
                  ...
                  await prisma.$transaction(
                    async (prisma2) => {
                      const databaseEntry =
                        await prisma2.paymentRequest.findMany({
                          where: {
                            identifier: decodedNewContract.referenceId,
                            networkHandlerId: networkCheck.id,
                            status:
                              $Enums.PaymentRequestStatus.PaymentRequested,
                          },
                          include: {
                            Amounts: true,
                          },
                      ...
                      const valueMatches = databaseEntry[0].Amounts.every(
                        (x) => {
                          const existingAmount = output.amount.find(
                            (y) => y.unit == x.unit,
                          );
                      ...
                      let newStatus =
                        $Enums.PaymentRequestStatus.PaymentInvalid;
                      if (valueMatches == true) {
                        newStatus =
                          $Enums.PaymentRequestStatus.PaymentConfirmed;
                      }
                      await prisma2.paymentRequest.update({
                        where: { id: databaseEntry[0].id },
                        data: {
                ...
                const decodedOutputDatum =
                  outputDatum != null ? Data.from(outputDatum) : null;
                const decodedNewContract =
                  decodeV1ContractDatum(decodedOutputDatum);
                ...
                if (redeemerVersion == 0) {
                  ...
                  if (decodedNewContract?.resultHash) {
                    ...
                    newStatus = await prisma.$transaction(
                      async (prisma2) => {
                          ...
                          for (const entry of databaseEntry) {
                            await prisma2.paymentRequest.update({
                              ...
                              data: {
                                ...
                                errorNote: 'Duplicate payment transaction',
                                errorType:
                                  $Enums.PaymentRequestErrorType.UNKNOWN,
                              },
                            });
                          }
                          return $Enums.PaymentRequestStatus.PaymentInvalid;
                        ...
                        const valueMatches = databaseEntry[0].Amounts.every(
                          (x) => {
                            const existingAmount = valueOutputs[0].amount.find(
                              (y) => y.unit == x.unit,
                            );
                            if (existingAmount == null) return false;
                            return (
                              x.amount.toString() == existingAmount.quantity
                            );
                          },
                        );
                        let newStatus2 =
                  ...
                  newPurchasingStatus =
                    $Enums.PurchasingRequestStatus.RefundConfirmed;
                } else if (redeemerVersion == 4) {
                  newStatus = $Enums.PaymentRequestStatus.DisputedWithdrawn;
                  newPurchasingStatus =
                    $Enums.PurchasingRequestStatus.DisputedWithdrawn;
                  ...
                  await prisma.$transaction(
                    async (prisma2) => {
                      const paymentRequest =
                        await prisma2.paymentRequest.findUnique({
                          where: {
                            networkHandlerId_identifier: {
                              networkHandlerId: networkCheck.id,
                              identifier: decodedOldContract.referenceId,
                            },
                          },
                        });
                ...
                await Promise.allSettled([
                  handlePaymentTransactionCardanoV1(
                    ...
                    decodedOldContract.referenceId,
                  ),
            ...
            await prisma.networkHandler.update({
              where: { id: networkCheck.id },
              data: {
                lastIdentifierChecked: latestTx[latestTx.length - 1].tx_hash,
                lastPageChecked: latestPage,
              },
            });
            if (latestTx.length >= 25) {
              latestPage++;
              latestTx = await blockfrost.addressesTransactions(
                networkCheck.paymentContractAddress,
                { count: 25, page: latestPage },
              );
            } else {
              latestTx = [];
            }
      ...
      try {
        ...
        logger.error('Error updating network checks syncing status', {
          error,
          networkChecks,
        });
      }
  ...
  await prisma.$transaction(
    async (prisma2) => {
      const paymentRequest = await prisma2.paymentRequest.findUnique({
        where: {
          networkHandlerId_identifier: {
            networkHandlerId: networkCheckId,
...
function decodeV1ContractDatum(decodedDatum) {
  ...
  const referenceId = Buffer.from(decodedDatum.fields[2], 'hex').toString(
    'utf-8',
  );
  if (typeof decodedDatum.fields[3] !== 'string') {
    return null;
  }
  const resultHash = Buffer.from(decodedDatum.fields[3], 'hex').toString(
    'utf-8',
  );
  if (
    typeof decodedDatum.fields[4] !== 'number' &&
    typeof decodedDatum.fields[4] !== 'bigint'
  ) {
    return null;
  }
  if (
    typeof decodedDatum.fields[5] !== 'number' &&
    typeof decodedDatum.fields[5] !== 'bigint'
  ) {
    return null;
  }
  if (
    typeof decodedDatum.fields[6] !== 'number' &&
    typeof decodedDatum.fields[6] !== 'bigint'
  ) {
    return null;
  }
...
function mBoolToBool(value) {
  ...
  if (
    value.index == bTrue.alternative &&
    typeof value.fields == typeof bTrue.fields
  ) {
    return true;
  }
  if (
    value.index == bFalse.alternative &&
    typeof value.fields == typeof bFalse.fields
  ) {
    return false;
  }
  return null;
}
const cardanoTxHandlerService = {
...
function decrypt(secretEncrypted) {
  ...
  return (
    decryptionCipher.update(encryptedData, void 0, 'utf8') +
...
async function batchLatestPaymentEntriesV1() {
  ...
  if (!acquiredMutex) return await updateMutex$5.acquire();
  try {
    const networkChecksWithWalletLocked = await prisma.$transaction(
      async (prisma2) => {
        ...
        for (const networkCheck of networkChecks) {
          for (const purchasingWallet of networkCheck.PurchasingWallets) {
            if (purchasingWallet.id) {
              ...
              logger.warn('No purchasing wallet found for purchase request', {
                purchasingWallet,
              });
            }
          }
    ...
    await Promise.allSettled(
      networkChecksWithWalletLocked.map(async (networkCheck) => {
        const network =
          networkCheck.network == 'MAINNET'
            ? 'mainnet'
            : networkCheck.network == 'PREPROD'
              ? 'preprod'
              : null;
        if (!network) throw new Error('Invalid network');
        const blockchainHandler = new BlockfrostProvider(
          networkCheck.rpcProviderApiKey,
          0,
        );
        ...
        for (const walletData of walletAmounts) {
          ...
          while (paymentRequestsRemaining.length > 0) {
            ...
            if (lovelaceRequired == -1) {
              ...
              paymentRequest.Amounts.push({
                unit: 'lovelace',
                amount: minTransactionCalculation + result[0].amount,
              });
            }
            ...
            if (isFulfilled) {
              ...
              for (const paymentAmount of paymentRequest.Amounts) {
                ...
                walletAmount.quantity -= parseInt(
        ...
        await Promise.allSettled(
          walletPairings.map(async (walletPairing) => {
            try {
              ...
              const unsignedTx = await new Transaction$1({
                initiator: wallet,
              }).setMetadata(674, {
              ...
              for (const paymentRequest of batchedRequests) {
                ...
                const lovelaceIndex = correctedPaymentAmounts.findIndex(
                  (amount) => amount.unit.toLowerCase() == 'lovelace',
                );
                if (lovelaceIndex != -1) {
                  const removedLovelace = correctedPaymentAmounts.splice(
                    lovelaceIndex,
                    1,
                  );
                  if (removedLovelace[0].amount > minTransactionCalculation) {
                    correctedPaymentAmounts.push({
                      unit: 'lovelace',
                      amount:
                        removedLovelace[0].amount - minTransactionCalculation,
                    });
                  }
                }
                const datum = {
              ...
              try {
                  ...
                  batchedRequests.map(async (request) => {
                    await prisma.purchaseRequest.update({
                      ...
                      data: {
                        SmartContractWallet: {
                          update: {
                            PendingTransaction: { update: { hash: txHash } },
                          },
                        },
                ...
                if (failedPurchaseRequests.length > 0) {
                  logger.error(
                    'Error updating payment status, retrying ',
                ...
                const retriedFailedRequests = failedRequests.filter(
                  (x) => x.status != 'fulfilled',
                );
                if (retriedFailedRequests.length > 0) {
                  logger.error(
                    'Error updating payment status while retrying ',
                    error,
                    retriedFailedRequests,
                  );
                }
              }
            } catch (error) {
...
var validators$1 = [
  {
    ...
    compiledCode:
      '',
    hash: '3206491c426a241b8abbc8c4d956e9a3d1b3b07e60753ec993d806dd',
  },
  {
...
var validators = [
  {
    ...
    compiledCode:
      '',
...
async function getPaymentScriptFromNetworkHandlerV1(networkCheckSupported) {
  ...
  return await getPaymentScriptV1(
    ...
    networkCheckSupported.network,
  );
}
async function getRegistryScriptFromNetworkHandlerV1(networkCheckSupported) {
  return await getRegistryScriptV1(
    networkCheckSupported.paymentContractAddress,
    networkCheckSupported.network,
  );
}
async function getPaymentScriptV1(
  adminWalletAddress1,
  adminWalletAddress2,
  adminWalletAddress3,
  feeWalletAddress,
...
async function collectOutstandingPaymentsV1() {
  ...
  if (!acquiredMutex) return await updateMutex$4.acquire();
  try {
    const networkChecksWithWalletLocked = await prisma.$transaction(
      async (prisma2) => {
        const networkChecks = await prisma2.networkHandler.findMany({
        ...
        const sellingWalletIds = networkChecks
          .map((x) => x.PaymentRequests)
          .flat()
          .map((x) => x.SmartContractWallet?.id);
    ...
    await Promise.allSettled(
      networkChecksWithWalletLocked.map(async (networkCheck) => {
        if (
          networkCheck.PaymentRequests.length == 0 ||
          networkCheck.CollectionWallet == null
        )
          return;
        const network =
          networkCheck.network == 'MAINNET'
        ...
        for (const request of paymentRequests) {
          if (request.smartContractWalletId == null) continue;
          if (
            deDuplicatedRequests.some(
              (r) => r.smartContractWalletId == request.smartContractWalletId,
            )
          )
            continue;
          ...
          deDuplicatedRequests.map(async (request) => {
            try {
              ...
              const { script, smartContractAddress } =
                await getPaymentScriptFromNetworkHandlerV1(networkCheck);
              const utxos = await wallet.getUtxos();
              if (utxos.length === 0) {
                throw new Error(
                  'No UTXOs found in the wallet. Wallet is empty.',
                );
              }
              const utxoByHash = await blockchainProvider.fetchUTxOs(
                request.txHash,
              );
              const utxo = utxoByHash.find(
                (utxo2) => utxo2.input.txHash == request.txHash,
              );
              if (!utxo) {
              ...
              for (const assetValue of utxo.output.amount) {
                ...
                const feeValue = BigInt(
                  Math.max(
                    minFee,
                    (Number(value) * networkCheck.feePermille) / 1e3,
                  ),
                );
              ...
              if (networkCheck.CollectionWallet == null) {
                await prisma.paymentRequest.update({
                  ...
                  data: {
                    errorType: 'UNKNOWN',
                    errorRequiresManualReview: true,
                    errorNote: 'Collection wallet not found',
                  },
                });
                throw new Error('Collection wallet not found');
              }
              const unsignedTx = new Transaction$1({ initiator: wallet })
                .setMetadata(674, {
                ...
                .sendAssets(
                  {
                    address:
                      networkCheck.FeeReceiverNetworkWallet.walletAddress,
                    datum,
                  },
              ...
              if (
                request.errorRetries == null ||
                request.errorRetries < networkCheck.maxCollectionRetries
              ) {
                await prisma.paymentRequest.update({
                  where: { id: request.id },
                ...
                const errorMessage =
                  'Error creating refund transaction: ' +
...
async function collectRefundV1() {
  ...
  if (!acquiredMutex) return await updateMutex$3.acquire();
  try {
    const networkChecksWithWalletLocked = await prisma.$transaction(
      async (prisma2) => {
        const networkChecks = await prisma2.networkHandler.findMany({
        ...
        for (const networkCheck of networkChecks) {
          for (const purchaseRequest of networkCheck.PurchaseRequests) {
            if (purchaseRequest.SmartContractWallet?.id) {
              ...
              logger.warn(
                'No smart contract wallet found for purchase request',
                { purchaseRequest },
              );
            }
          }
    ...
    await Promise.allSettled(
      networkChecksWithWalletLocked.map(async (networkCheck) => {
        if (
          networkCheck.PurchaseRequests.length == 0 ||
          networkCheck.CollectionWallet == null
        )
          return;
        const network =
            ...
            try {
              ...
              await prisma.purchaseRequest.update({
                ...
                data: {
                  ...
                  SmartContractWallet: {
                    update: {
                      PendingTransaction: { create: { hash: txHash } },
                    },
                  },
                },
              });
              ...
              if (
                request.errorRetries == null ||
                request.errorRetries < networkCheck.maxRefundRetries
              ) {
                await prisma.paymentRequest.update({
                  where: { id: request.id },
                  data: { errorRetries: { increment: 1 } },
...
async function submitResultV1() {
  ...
  if (!acquiredMutex) return await updateMutex$2.acquire();
  try {
    const networkChecksWithWalletLocked = await prisma.$transaction(
      async (prisma2) => {
        const networkChecks = await prisma2.networkHandler.findMany({
        ...
        await Promise.allSettled(
          deDuplicatedRequests.map(async (request) => {
            try {
              ...
              if (
                request.errorRetries == null ||
                request.errorRetries < networkCheck.maxCollectRefundRetries
              ) {
                await prisma.paymentRequest.update({
                  where: { id: request.id },
                  data: { errorRetries: { increment: 1 } },
...
async function updateWalletTransactionHash() {
  ...
  if (!acquiredMutex) return await updateMutex$1.acquire();
  try {
    ...
    await Promise.allSettled(
      lockedPurchaseWallets.map(async (wallet) => {
        try {
    ...
    const timedOutLockedPurchaseWallets =
      await prisma.purchasingWallet.findMany({
        where: {
          PendingTransaction: {
            updatedAt: {
              //wallets that have not been updated in the last 20 minutes
              lt: new Date(Date.now() - 1e3 * 60 * 20),
            },
          },
        },
    ...
    await Promise.allSettled(
      timedOutLockedPurchaseWallets.map(async (wallet) => {
        try {
          const txHash = wallet.PendingTransaction?.hash;
          if (txHash) {
    ...
    if (
      timedOutLockedPurchaseWallets.length > 0 ||
      lockedPurchaseWallets.length > 0
    ) {
      try {
    ...
    await Promise.allSettled(
      lockedSellingWallets.map(async (wallet) => {
        try {
          const txHash = wallet.PendingTransaction.hash;
          const blockfrostKey = wallet.NetworkHandler.rpcProviderApiKey;
          const provider = new BlockfrostProvider(blockfrostKey);
    ...
    const timedOutLockedSellingWallets = await prisma.sellingWallet.findMany({
      where: {
        PendingTransaction: {
          updatedAt: { lt: new Date(Date.now() - 1e3 * 60 * 20) },
        },
      },
      include: { PendingTransaction: true },
    });
    await Promise.allSettled(
      timedOutLockedSellingWallets.map(async (wallet) => {
        try {
          const txHash = wallet.PendingTransaction?.hash;
          if (txHash) {
    ...
    if (
      timedOutLockedSellingWallets.length > 0 ||
      lockedSellingWallets.length > 0
    ) {
    }
  } finally {
...
const updateWalletTransactionHashHandlerService = {
...
async function init() {
  ...
  cron.schedule(CONFIG.CHECK_TX_INTERVAL, async () => {
    ...
    logger.info(
      'finished updating cardano payment entries in ' +
        (/* @__PURE__ */ new Date().getTime() - start.getTime()) / 1e3 +
        's',
    );
  });
  ...
  cron.schedule(CONFIG.CHECK_COLLECTION_INTERVAL, async () => {
    ...
    logger.info(
      'finished checking payments to collect in ' +
        (/* @__PURE__ */ new Date().getTime() - start.getTime()) / 1e3 +
        's',
    );
  });
  ...
  cron.schedule(CONFIG.CHECK_REFUND_INTERVAL, async () => {
    ...
    logger.info(
      'finished checking payments to refund in ' +
        (/* @__PURE__ */ new Date().getTime() - start.getTime()) / 1e3 +
        's',
    );
  });
  cron.schedule(CONFIG.CHECK_WALLET_TRANSACTION_HASH_INTERVAL, async () => {
    ...
    logger.info(
      'finished checking wallet transaction hash in ' +
        (/* @__PURE__ */ new Date().getTime() - start.getTime()) / 1e3 +
        's',
    );
  });
...
const authMiddleware = (minPermission) =>
  new Middleware({
    security: {
    ...
    handler: async ({ request, logger }) => {
      ...
      if (
        minPermission == $Enums.Permission.ADMIN &&
        apiKey.permission != $Enums.Permission.ADMIN
      ) {
        throw createHttpError(401, 'Unauthorized, admin access required');
      }
      if (
        minPermission == $Enums.Permission.READ_PAY &&
        apiKey.permission != $Enums.Permission.READ_PAY &&
        apiKey.permission != $Enums.Permission.ADMIN
      ) {
...
const adminAuthenticatedEndpointFactory = defaultEndpointsFactory.addMiddleware(
...
const queryAPIKeyEndpointGet = adminAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input }) => {
    const result = await prisma.apiKey.findMany({
      where: {},
      cursor: input.cursorApiKey ? { apiKey: input.cursorApiKey } : void 0,
      take: input.limit,
        ...
        return {
          ...
          RemainingUsageCredits: data.RemainingUsageCredits.map(
            (usageCredit) => ({
              ...
              amount: parseInt(usageCredit.amount.toString()),
            }),
          ),
        };
      }),
...
const addAPIKeyEndpointPost = adminAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input }) => {
    const apiKey =
      ('masumi-registry-' + input.permission == $Enums.Permission.ADMIN
        ? 'admin-'
        : '') + createId();
...
const updateAPIKeySchemaInput = z.object({
  ...
  apiKey: z
    ...
    .max(550)
    .optional()
    .describe('The API key to update. Provide either id or apiKey'),
  UsageCredits: z
    .array(
      z.object({
        unit: z.string().max(150),
...
const deleteAPIKeySchemaInput = z.object({
  id: z
    ...
    .max(150)
    .optional()
    .describe('The id of the API key to delete. Provide either id or apiKey'),
  apiKey: z
    .string()
    .max(550)
    .optional()
    .describe('The API key to delete. Provide either id or apiKey'),
});
...
const payAuthenticatedEndpointFactory = defaultEndpointsFactory.addMiddleware(
  authMiddleware('READ_PAY'),
);
...
async function handlePurchaseCreditInit$1(
  id,
  cost,
  network,
  identifier,
...
const creditTokenRepository = {
  handlePurchaseCreditInit: handlePurchaseCreditInit$1,
};
...
const queryPurchaseRequestSchemaInput = z.object({
  ...
  cursorIdentifierSellingWalletVkey: z
    .string()
    .max(250)
    .optional()
    .describe(
  ...
  purchases: z.array(
    z.object({
      ...
      SmartContractWallet: z
        ...
        .nullable(),
      SellerWallet: z
        .object({ walletVkey: z.string(), note: z.string().nullable() })
        .nullable(),
      Amounts: z.array(
        z.object({
          id: z.string(),
...
const queryPurchaseRequestGet = payAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input, logger }) => {
    ...
    if (input.cursorIdentifierSellingWalletVkey && input.cursorIdentifier) {
      ...
      cursor = {
        networkHandlerId_identifier_sellerWalletId: {
          networkHandlerId: networkHandler.id,
...
const createPurchaseInitSchemaInput = z.object({
  ...
  amounts: z
    .array(
      z.object({
        amount: z.number({ coerce: true }).min(0).max(Number.MAX_SAFE_INTEGER),
        unit: z.string(),
      }),
    )
    .max(7)
  ...
  paymentType: z
    .nativeEnum($Enums.PaymentType)
    .describe('The payment type of smart contract used'),
  unlockTime: ez
    .dateIn()
    .describe('The time after which the purchase will be unlocked'),
  refundTime: ez
    .dateIn()
    .describe('The time after which a refund will be approved'),
  submitResultTime: ez
    .dateIn()
    .describe('The time by which the result has to be submitted'),
});
...
const createPurchaseInitPost = payAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input, options, logger }) => {
    ...
    const wallets = await prisma.purchasingWallet.aggregate({
      where: { networkHandlerId: networkCheckSupported.id },
      _count: true,
    });
    if (wallets._count === 0) {
    ...
    if (
      input.unlockTime >
      new Date(input.refundTime.getTime() + additionalRefundTime)
    ) {
      throw createHttpError(
        400,
        'Refund request time must be after unlock time with at least 3 hours difference',
      );
    }
    ...
    if (
      input.submitResultTime > new Date(input.unlockTime.getTime() + offset)
    ) {
      throw createHttpError(
        400,
        'Submit result time must be after unlock time with at least 15 minutes difference',
      );
    }
    const initial = await tokenCreditService.handlePurchaseCreditInit(
      options.id,
...
const refundPurchasePatch = payAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input, logger }) => {
    ...
    const address = (await wallet.getUnusedAddresses())[0];
    const { script, smartContractAddress } =
      await getPaymentScriptFromNetworkHandlerV1(networkCheckSupported);
    const utxos = await wallet.getUtxos();
    if (utxos.length === 0) {
      throw new Error('No UTXOs found in the wallet. Wallet is empty.');
    }
    const utxoByHash = await blockchainProvider.fetchUTxOs(purchase.txHash);
    const utxo = utxoByHash.find(
      (utxo2) => utxo2.input.txHash == purchase.txHash,
    );
    if (!utxo) {
      throw new Error('UTXO not found');
    }
    ...
    const resultHash = Buffer.from(decodedDatum.value[3], 'hex').toString(
      'utf-8',
    );
    if (typeof decodedDatum.value[4] !== 'number') {
      throw new Error('Invalid datum at position 4');
    ...
    const unlockTime = decodedDatum.value[5];
    const refundTime = decodedDatum.value[6];
    const refundDenied = cardanoTxHandlerService.mBoolToBool(
      decodedDatum.value[8],
    );
    ...
    const networkType =
      networkCheckSupported.network == 'MAINNET' ? 'mainnet' : 'preprod';
    const invalidBefore =
      unixTimeToEnclosingSlot(
        Date.now() - 15e4,
        SLOT_CONFIG_NETWORK[networkType],
      ) - 1;
    ...
    const unsignedTx = new Transaction$1({ initiator: wallet })
      ...
      .sendValue({ address: smartContractAddress, datum }, utxo)
      .setChangeAddress(address)
      .setRequiredSigners([address]);
    unsignedTx.txBuilder.invalidBefore(invalidBefore);
...
const authenticatedEndpointFactory = defaultEndpointsFactory.addMiddleware(
  authMiddleware('READ'),
);
...
const queryPaymentsSchemaInput = z.object({
  ...
  cursorIdentifier: z
    ...
    .max(250)
    .optional()
    .describe('Used to paginate through the payments'),
  network: z
    .nativeEnum($Enums.Network)
...
const createPaymentsSchemaInput = z.object({
  ...
  submitResultTime: ez
    .dateIn()
    .default(new Date(Date.now() + 1e3 * 60 * 60 * 12).toISOString())
    .describe(
...
const paymentInitPost = authenticatedEndpointFactory.build({
  ...
  handler: async ({ input, logger }) => {
    ...
    const unlockTime =
      input.unlockTime != void 0
        ? input.unlockTime.getTime()
        : new Date(Date.now() + 1e3 * 60 * 60 * 12).getTime();
    const refundTime =
      input.refundTime != void 0
        ? input.refundTime.getTime()
        : new Date(Date.now() + 1e3 * 60 * 60 * 24).getTime();
    ...
    if (
      networkCheckSupported.SellingWallets.find(
        (wallet) => wallet.walletVkey == vKey,
      ) == null
    ) {
  ...
  handler: async ({ input, logger }) => {
    ...
    const networkCheckSupported = await prisma.networkHandler.findUnique({
      ...
      include: {
        SellingWallets: true,
        CollectionWallet: true,
        PaymentRequests: { where: { identifier: input.identifier } },
      },
    });
    if (networkCheckSupported == null) {
      throw createHttpError(
    ...
    if (
      networkCheckSupported.PaymentRequests[0].status !=
      $Enums.PaymentRequestStatus.PaymentConfirmed
    ) {
      throw createHttpError(
        400,
        'Payment in invalid state ' +
          networkCheckSupported.PaymentRequests[0].status,
      );
    }
...
const registerAgentPost = payAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input, logger }) => {
    ...
    const networkCheckSupported = await prisma.networkHandler.findUnique({
      ...
      include: {
        AdminWallets: true,
        SellingWallets: { include: { WalletSecret: true } },
      },
    });
    if (networkCheckSupported == null) {
      throw createHttpError(
    ...
    if (
      networkCheckSupported.SellingWallets == null ||
      networkCheckSupported.SellingWallets.length == 0
    ) {
    ...
    let sellingWallet = networkCheckSupported.SellingWallets.find(
      (wallet2) => wallet2.walletVkey == input.sellingWalletVkey,
    );
    if (sellingWallet == null) {
      ...
      const randomIndex = Math.floor(
        Math.random() * networkCheckSupported.SellingWallets.length,
      );
    ...
    const address = (await wallet.getUnusedAddresses())[0];
    const { script, policyId, smartContractAddress } =
      await getRegistryScriptFromNetworkHandlerV1(networkCheckSupported);
    const utxos = await wallet.getUtxos();
    if (utxos.length === 0) {
    ...
    const tx = new Transaction$1({ initiator: wallet })
      .setMetadata(674, {
      ...
      [policyId]: {
        [assetName]: {
          ...
          author: {
            ...
            contact: input.author.contact
              ? stringToMetadata(input.author.contact)
              : void 0,
            organization: input.author.organization
              ? stringToMetadata(input.author.organization)
              : void 0,
          },
          legal: input.legal
            ? {
                privacy_policy: input.legal?.privacy_policy
                  ? stringToMetadata(input.legal.privacy_policy)
                  : void 0,
                terms: input.legal?.terms
                  ? stringToMetadata(input.legal.terms)
                  : void 0,
                other: input.legal?.other
                  ? stringToMetadata(input.legal.other)
                  : void 0,
              }
            : void 0,
          tags: input.tags,
...
const unregisterAgentDelete = payAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input, logger }) => {
    ...
    const { policyId, script, smartContractAddress } =
      await getRegistryScriptFromNetworkHandlerV1(networkCheckSupported);
    const holderWallet = await blockfrost.assetsAddresses(
      policyId + input.assetName,
      { order: 'desc', count: 1 },
    );
    ...
    const sellingWallet = networkCheckSupported.SellingWallets.find(
      (wallet2) => wallet2.walletVkey == vkey,
    );
    if (sellingWallet == null) {
      ...
      const sellingWallets = await Promise.all(
        sellingWalletsMesh.map(async (sw) => {
          return {
            ...
            walletVkey: resolvePaymentKeyHash$1(
              (await sw.wallet.getUnusedAddresses())[0],
            ),
            walletSecretId: (
              await prisma2.walletSecret.create({ data: { secret: sw.secret } })
            ).id,
            note: sw.note,
          };
        }),
      ...
      const purchasingWallets = await Promise.all(
        purchasingWalletsMesh.map(async (pw) => {
          return {
            ...
            walletSecretId: (
              await prisma2.walletSecret.create({ data: { secret: pw.secret } })
            ).id,
            note: pw.note,
          };
        }),
    ...
    const result = await prisma.$transaction(async (prisma2) => {
          ...
          ? await Promise.all(
              sellingWalletsMesh.map(async (sw) => {
                return {
                  ...
                  walletSecretId: (
                    await prisma2.walletSecret.create({
                      data: { secret: sw.secret },
                    })
                  ).id,
                  note: sw.note,
                };
      ...
      const purchasingWallets =
        purchasingWalletsMesh != null
          ...
          : [];
      if (
        (input.RemoveSellingWallets != null &&
          input.RemoveSellingWallets.length > 0) ||
        (input.RemovePurchasingWallets != null &&
          input.RemovePurchasingWallets.length > 0)
      ) {
      ...
      const paymentSource = await prisma2.networkHandler.update({
        ...
        data: {
          ...
          rpcProviderApiKey: input.rpcProviderApiKey,
          CollectionWallet:
            input.CollectionWallet != void 0
              ? {
                  update: {
                    walletAddress: input.CollectionWallet.walletAddress,
                    note: input.CollectionWallet.note,
                  },
                }
              : void 0,
          SellingWallets: {
...
const queryWalletEndpointGet = adminAuthenticatedEndpointFactory.build({
  ...
  handler: async ({ input }) => {
    if (input.walletType == 'Selling') {
      const result = await prisma.sellingWallet.findFirst({
        where: { id: input.id },
        include: {
          WalletSecret: true,
      ...
      const result = await prisma.purchasingWallet.findFirst({
        where: { id: input.id },
        include: {
          WalletSecret: true,
          PendingTransaction: true,
...
const queryRpcProviderKeysEndpointGet = adminAuthenticatedEndpointFactory.build(
  {
    ...
    handler: async ({ input }) => {
      const rpcProviderKeys = await prisma.networkHandler.findMany({
        cursor: input.cursorId ? { id: input.cursorId } : void 0,
...
const queryUTXOEndpointGet = authenticatedEndpointFactory.build({
  ...
  handler: async ({ input }) => {
    const result = await prisma.networkHandler.findFirst({
    ...
    const blockfrost = new BlockFrostAPI({
      projectId: result.rpcProviderApiKey,
    });
              ...
              .openapi({
                example: {
                  ...
                  data: {
                    ...
                    usageLimited: true,
                    RemainingUsageCredits: [{ unit: 'lovelcae', amount: 1e6 }],
                  },
                },
              }),
          },
                    ...
                    payments: [
                      {
                        ...
                        NetworkHandler: {
                          id: 'unique_cuid_v2_auto_generated',
                          network: $Enums.Network.PREPROD,
                          paymentContractAddress: 'address_to_check',
                          paymentType: $Enums.PaymentType.WEB3_CARDANO_V1,
                        },
                      },
                    ],
            ...
            schema: z
              ...
              .openapi({
                example: {
                  ...
                  data: {
                    identifier:
                      'agent_identifier_unique_cuid_v2_auto_generated',
                    createdAt: /* @__PURE__ */ new Date(),
                    updatedAt: /* @__PURE__ */ new Date(),
                    status: $Enums.PaymentRequestStatus.PaymentRequested,
                  },
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: z
              .object({ data: updatePaymentSchemaOutput, status: z.string() })
              .openapi({
                example: {
                  status: 'success',
                  data: {
                    ...
                    purchases: [
                      {
                        ...
                        updatedAt: /* @__PURE__ */ new Date(),
                        status:
                          $Enums.PurchasingRequestStatus.PurchaseRequested,
                        txHash: 'tx_hash',
                        utxo: 'utxo',
                        errorType:
                          $Enums.PurchaseRequestErrorType.NETWORK_ERROR,
                        errorNote: 'error_note',
                        errorRequiresManualReview: false,
                        identifier: 'identifier',
                        SmartContractWallet: {
    ...
    responses: {
      200: {
        ...
        content: {
          'application/json': {
            schema: z
              .object({ data: refundPurchaseSchemaOutput, status: z.string() })
              .openapi({
                example: {
                  status: 'success',
                  data: {
          ...
          'application/json': {
            schema: paymentSourceCreateSchemaInput.openapi({
              example: {
                ...
                PurchasingWallets: [
                  { walletMnemonic: 'wallet mnemonic', note: 'note' },
                ],
                SellingWallets: [
                  { walletMnemonic: 'wallet mnemonic', note: 'note' },
                ],
        ...
        content: {
          'application/json': {
            schema: z
              .object({
                status: z.string(),
                data: paymentSourceCreateSchemaOutput,
              })
              .openapi({
                example: {
                  status: 'success',
            ...
            schema: paymentSourceUpdateSchemaInput.openapi({
              example: {
                ...
                AddPurchasingWallets: [
                  { walletMnemonic: 'wallet_mnemonic', note: 'note' },
                ],
                AddSellingWallets: [
                  { walletMnemonic: 'wallet_mnemonic', note: 'note' },
                ],
        ...
        content: {
          'application/json': {
            schema: z
              .object({
                status: z.string(),
                data: paymentSourceUpdateSchemaOutput,
              })
              .openapi({
                example: {
                  status: 'success',
        ...
        content: {
          'application/json': {
            schema: z
              .object({
                status: z.string(),
                data: paymentSourceDeleteSchemaOutput,
              })
              .openapi({
                example: {
                  status: 'success',
                ...
                utxos: [
                  {
                    txHash: 'tx_hash',
                    address: 'addr1qx2ej34k567890',
                    amount: [{ unit: 'lovelace', quantity: 1e6 }],
                    output_index: 1,
                    block: '1',
                  },
                ],
...
initialize()
  .then(async () => {
    ...
    const serverConfig = createConfig({
      ...
      beforeRouting: ({ app }) => {
        logger.info(
          'Serving the API documentation at localhost:' + PORT + '/docs',
        );
        app.use(
          '/docs',
          ui.serve,
          ui.setup(generateOpenAPI(), {
            explorer: false,
            swaggerOptions: {
