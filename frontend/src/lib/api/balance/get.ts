...
export async function getWalletBalance(
  ...
  try {
    ...
    const response = await fetch(
      `${networkUrl}/addresses/${options.walletAddress}`,
      {
        headers: {
    ...
    if (!response.ok) {
      throw new Error(
        `Blockfrost API request failed with status ${response.status}: ${await response.text()}`,
      );
    }
    ...
    const balanceUsdm = data.amount.reduce(
      (total: number, asset: BlockfrostAsset) => {
    ...
    const balanceNmkr = data.amount.reduce(
      (total: number, asset: BlockfrostAsset) => {
